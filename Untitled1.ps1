...
for ($i=0; $i -lt $arrayRepos.Count; $i++) {
    ...
    git ls-files | Where-object {$_ -like "*.cs"}| Where-object {$_ -like "*EMERBatch/Pgms*"}| foreach{",Oh/Odjfs/Sets/$_".replace('/','.')} | foreach{$_ -replace '.cs'} | foreach{$_.Split(".")[-1]+$_} | out-file -Append $metadatapath\temp.txt
    git ls-files | Where-object {$_ -like "*.cs"}| Where-object {$_ -like "*BatchCore/Pgms*"}| foreach{",Oh/Odjfs/Sets/$_".replace('/','.')} | foreach{$_ -replace '.cs'} | foreach{$_.Split(".")[-1]+$_} | out-file -Append $metadatapath\temp.txt
    git ls-files | Where-object {$_ -like "*.cs"}| Where-object {$_ -like "*Common/Pgms*"}| foreach{",Oh/Odjfs/Sets/$_".replace('/','.')} | foreach{$_ -replace '.cs'} | foreach{$_.Split(".")[-1]+$_} | out-file -Append $metadatapath\temp.txt
...
if([String]::IsNullOrWhiteSpace((Get-content $metadatapath\difference.txt))){
        ...
        Clear-Content $metadatapath\programmetadata.txt
